...
def get_db():
    ...
    if "db" not in g:
        ...
        g.db = DatabaseManager(
            db_folder_path=db_folder_path, db_path=database_path
        )
    return g.db
...
def update_agreement():
    ...
    if request.form.get("agreement"):
        db_manager.update_setting(
            "user_consent", int(bool((request.form.get("agreement"))))
        )
...
def settings():
    ...
    selected_settings.update(
        {"media_types": media_types, "drm_types": drm_types}
    )
    selected_settings["use_custom_ffmpeg"] = bool(
        selected_settings["use_custom_ffmpeg"]
    )
...
def update_settings():
    ...
    if request.form.get("download_from_players"):
        db_manager.update_setting(
            "download_from_players", request.form.get("download_from_players")
        )
    if request.form.get("download_drm_content"):
        db_manager.update_setting(
            "download_drm_content", request.form.get("download_drm_content")
        )
    if request.form.get("download_drm_types"):
        db_manager.update_setting(
            "download_drm_types", request.form.get("download_drm_types")
        )
    if request.form.get("default_user_agent"):
        db_manager.update_setting(
            "default_user_agent", request.form.get("default_user_agent")
        )
    if request.form.get("use_custom_ffmpeg"):
        db_manager.update_setting(
            "use_custom_ffmpeg", request.form.get("use_custom_ffmpeg")
        )
    if request.form.get("custom_ffmpeg_path"):
        db_manager.update_setting(
            "custom_ffmpeg_path", request.form.get("custom_ffmpeg_path")
        )
